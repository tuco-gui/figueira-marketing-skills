...
Este passo depende de o ambiente de execução atual conseguir rodar Chromium headless (verificar no host atual). **Se por algum motivo o Chromium não estiver disponível no ambiente, o Da Vinci deve avisar o usuário em vez de falhar silenciosamente** — entregar os HTMLs + o `render.py` + instruções, e explicar que o passo de imagem não pôde rodar aqui.
